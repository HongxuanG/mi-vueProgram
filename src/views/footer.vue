<template>
    <div>
        <div class="header-footer">
            <div class="container">
                <div class="footer-server">
                    <ul class="clearfix">
                        <li>
                            <a href="#">
                                <span class="iconfont icon-xiuli"></span>
                                预约维修服务
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                <span class="iconfont icon-sevenday"></span>
                                7天无理由退货
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                <span class="iconfont icon-tian"></span>
                                15天免费换货
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                <span class="iconfont icon-lipin"></span>
                                满99元包邮
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                <span class="iconfont icon-didian"></span>
                                520余家售后网点
                            </a>
                        </li>
                    </ul>
                </div>
                <div class="footer-link clearfix">
                    <div class="link clearfix">
                        <dl>
                            <dt>帮助中心</dt>
                            <dd><a href="#">账号管理</a></dd>
                            <dd><a href="#">购物指南</a></dd>
                            <dd><a href="#">订单操作</a></dd>
                        </dl>
                        <dl>
                            <dt>服务支持</dt>
                            <dd><a href="#">售后政策</a></dd>
                            <dd><a href="#">自助服务</a></dd>
                            <dd><a href="#">相关下载</a></dd>
                        </dl>
                        <dl>
                            <dt>线下门店</dt>
                            <dd><a href="#">小米之家</a></dd>
                            <dd><a href="#">服务网点</a></dd>
                            <dd><a href="#">授权体验店</a></dd>
                        </dl>
                        <dl>
                            <dt>关于小米</dt>
                            <dd><a href="#">了解小米</a></dd>
                            <dd><a href="#">加入小米</a></dd>
                            <dd><a href="#">投资者关系</a></dd>
                            <dd><a href="#">企业社会责任</a></dd>
                            <dd><a href="#">廉洁举报</a></dd>
                        </dl>
                        <dl>
                            <dt>关注我们</dt>
                            <dd><a href="#">新浪微博</a></dd>
                            <dd><a href="#">官方微信</a></dd>
                            <dd><a href="#">联系我们</a></dd>
                            <dd><a href="#">公益基金会</a></dd>
                        </dl>
                        <dl>
                            <dt>特色服务</dt>
                            <dd><a href="#">F码通道</a></dd>
                            <dd><a href="#">礼物码</a></dd>
                            <dd><a href="#">防伪查询</a></dd>
                        </dl>
                    </div>
                    <div class="contact">
                        <div class="title">400-100-5678</div>
                        <p>8：00-18：00（仅收市话费）</p>
                        <button class="help">
                            <i class="iconfont icon-liaotian"></i>
                            人工客服
                        </button>
                        <p class="follow">
                            关注小米：<span class="iconfont icon-weibo"></span><span class="iconfont icon-weixin"></span>
                        </p>
                    </div>
                </div>
                <div class="info-text">
                    <p class="sites">
                        <a href="#">小米商城</a>
                        <span class="sep">|</span>
                        <a href="#">MIUI</a>
                        <span class="sep">|</span>
                        <a href="#">米家</a>
                        <span class="sep">|</span>
                        <a href="#">米聊</a>
                        <span class="sep">|</span>
                        <a href="#">多看</a>
                        <span class="sep">|</span>
                        <a href="#">游戏</a>
                        <span class="sep">|</span>
                        <a href="#">政企服务</a>
                        <span class="sep">|</span>
                        <a href="#">小米天猫店</a>
                        <span class="sep">|</span>
                        <a href="#">小米集团隐私政策</a>
                        <span class="sep">|</span>
                        <a href="#">小米公司儿童信息保护规则</a>
                        <span class="sep">|</span>
                        <a href="#">小米商城隐私政策</a>
                        <span class="sep">|</span>
                        <a href="#">小米商城用户协议</a>
                        <span class="sep">|</span>
                        <a href="#">问题反馈</a>
                        <span class="sep">|</span>
                        <a href="#">Select Location</a>
                        <span class="sep">|</span>
                    </p>
                    <p>
                        © <a href="#">mi.com</a> 京ICP证110507号 <a href="#">京ICP备10046444号</a> <a href="#">京公网安备11010802020134号</a> <a href="#">京网文[2020]0276-042号</a><br>
                        <a href="#">（京）网械平台备字（2018）第00005号</a> <a href="#">互联网药品信息服务资格证（京）非经营性-2014-0090</a> <a href="#">营业执照</a> <a href="#">医疗器械质量公告</a><br>
                        增值电信业务许可证网络食品经营备案（京）【2018】WLSPJYBAHF-12<a href="#">食品经营许可证</a><br>
                        违法和不良信息举报电话：185-0130-1238 <a href="#">知识产权侵权投诉</a>本网站所列数据，除特殊说明，所有数据均出自我司实验室测试
                    </p>
                </div>
                <div class="info-links">
                    <img src="../assets/img/truste.png" alt="">
                    <img src="../assets/img/v-logo-2.png" alt="">
                    <img src="../assets/img/v-logo-1.png" alt="">
                    <img src="../assets/img/ba10428a4f9495ac310fd0b5e0cf8370.png" alt="">
                </div>
            </div>
        </div>
    </div>
</template>

<script>
    export default {
        data(){
            return{
                
            }
        }
    }
</script>

<style lang="scss" scoped>
    $mainColor:#ff6700;
    .header-footer{
        background-color: #fff;
        .footer-server{
            border-bottom: 1px solid #e0e0e0;
            ul{
                li{
                    float: left;
                    width: calc((100% - 4px) / 5 );
                    padding: 25px 0;
                    text-align: center;
                    a{
                        display: block;
                        width: 100%;
                        border-right: 1px solid #e0e0e0;
                        transition: color 0.2s linear;
                        font-size: 16px;
                        color: #616161;
                        span{
                            font-size: 22px;
                            vertical-align: middle;
                        }
                        &:hover{
                            color: $mainColor;
                        }
                        
                    }
                    &:last-child{
                        a{
                            border:0px;
                        }
                    }
                }
            }
        }
        .footer-link{
            padding-top: 40px;
            .link{
                float: left;
                dl{
                    float: left;
                    width: 160px;
                    dt{
                        font-size: 14px;
                        margin-bottom: 30px;
                    }
                    dd{
                        margin-bottom: 15px;
                        a{

                        }
                    }
                }
            }
            .contact{
                float: right;
                width: 251px;
                border-left: 1px solid #e0e0e0;
                text-align: center;
                line-height: 25px;
                .title{
                    font-size: 20px;
                    color: $mainColor;
                }
                p{
                    
                }
                button.help{
                    outline: none;
                    width: 118px;
                    height: 28px;
                    background-color: #fff;
                    border: 1px solid $mainColor;
                    color: $mainColor;
                    font-size: 12px;
                    margin-bottom: 5px;
                    transition: all .2s linear;
                    cursor: pointer;
                    i{
                        font-size: 14px;
                        vertical-align: middle;
                    }
                    &:hover{
                        background-color: $mainColor;
                        color: #fff;
                    }
                }
                .follow{
                    span{
                        margin-left: 10px;
                        font-size: 18px;
                        vertical-align: middle;
                    }
                }
            }
            a:hover{
                color: $mainColor;
            }
        }
        .info-text{
            p.sites{
                font-size: 14px;
                margin: 10px 0;
                span.sep{
                    color: #333;
                    margin: 0 5px;
                }
                span.sep:last-child{
                    display: none;
                }
            }
            p{
                margin: 10px 0;
                font-size: 14px;
                color: #333;
                line-height: 20px;
                a{

                }
            }
            a:hover{
                color: $mainColor;
            }
        }
        .info-links{
            padding: 10px 0;
            img{
                width: auto;
                height: 30px;
                display: inline-block;
                vertical-align: baseline;
            }
            img:first-child{
                height: 57px;
            }
        }
    }
</style>